...
export async function GET(_: Request, context: { params: Promise<{ projectId: string }> }) {
  ...
  return proxyBackend({ path: `/v2/projects/${normalized}`, method: "GET" });
}
...
export async function PATCH(req: Request, context: { params: Promise<{ projectId: string }> }) {
  ...
  return proxyBackend({ path: `/v2/projects/${normalized}`, method: "PATCH", body: payload });
}
...
export async function DELETE(_: Request, context: { params: Promise<{ projectId: string }> }) {
  ...
  return proxyBackend({ path: `/v2/projects/${normalized}`, method: "DELETE" });
}
