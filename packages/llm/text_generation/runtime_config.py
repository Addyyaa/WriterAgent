...
class TextGenerationRuntimeConfig:
    ...
    def from_env(cls) -> "TextGenerationRuntimeConfig":
        return cls(
            use_mock=env_bool("WRITER_LLM_USE_MOCK", True),
            fallback_to_mock_on_error=env_bool("WRITER_LLM_FALLBACK_TO_MOCK", True),
            base_url=env_str("WRITER_LLM_BASE_URL", "https://api.openai.com/v1"),
            api_key=env_str("WRITER_LLM_API_KEY", ""),
            model=env_str("WRITER_LLM_MODEL", "gpt-4o-mini"),
            timeout_seconds=env_float("WRITER_LLM_TIMEOUT", 30.0, minimum=1.0, maximum=300.0),
